...
1. כתיב תקני (מלא/חסר ומילים שגויות):
   מיסגרות→מסגרות | אומנות→אמנות | דוגמא→דוגמה | וודאי→ודאי | זהותינו→זהותנו

2. מילים תקניות — השתמש במילה הנכונה:
   משטרה בהקשר שלטוני→משטר

3. התאמה דקדוקית — מין, מספר, זמן:
   הטענות מוצדק→הטענות מוצדקות

4. פסיקים — הוסף פסיק אחרי ביטויי פתיחה/הסגר:
   "בכך מבלי שאמרו דבר הם"→"בכך, מבלי שאמרו דבר, הם"

5. מקף בצירופים כבולים:
   כל יכול→כל-יכול

6. רווחים וסימני פיסוק:
   פסיק/נקודה צמודים למילה לפניהם; רווח אחריהם בלבד.

7. כינוי גוף חסר — אם נושא שמני ונשוא שמני רצופים ללא פועל, הוסף הוא/היא/הם/הן:
   "שיטת משטרה אוליגרכיה"→"שיטת משטרה היא אוליגרכיה"
   "הצבא כלי"→"הצבא הוא כלי"

8. החלפת "ש" כתחלית ב-"ה" כשאפשרי:
   "שמבקש לפרק"→"המבקש לפרק" | "שגוזרים קופון"→"הגוזרים קופון"

9. הוספת "האם" בשאלות שאינן נפתחות במילת שאלה.

══ כללי גרש ══

גרש יחיד (׳) = מושגים ומונחים בלבד. גרשיים (״) = ציטוט ישיר של דברי אנשים בלבד.

▸ כלל א — מילית יחס לפני גרש:
NEVER write: לה'...' / בה'...' / מה'...' / כה'...'
כשמילית יחס (ל/ב/מ/כ) קודמת למונח בגרש — ה"א נבלעת ונמחקת.
   לה'ברית היהודית'→ל'ברית היהודית' | בה'שמאל'→ב'שמאל' | מה'ימין'→מ'ימין'

▸ כלל ב — ה"א בתוך גרש:
NEVER write: '...' כשהאות הראשונה בתוך הגרש היא ה.
הוצא ה"א לפני הגרש הפותח ומחק מבפנים.
   'הדמוקרטיה'→ה'דמוקרטיה' | 'הברית היהודית'→ה'ברית היהודית'

▸ כלל ג — כפילות ה"א:
NEVER write: ה'ה...' (שתי ה"א).
...
══ חובה לשמור ══
- מבנה שורות ורווחים זהה למקור
- כוכביות (*) במקומן המדויק
