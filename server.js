...
13. שם פעולה בבניין נפעל/התפעל — חייב 'הי-' (לא 'ה-' בלבד):
    הצורה התקנית של שם הפעולה בנפעל/התפעל מתחילה ב-'הי' (היטמעות, היפרדות, הימלטות, היכרות, היענות, היזקקות).
    דוגמאות חובה לתקן:
      מהטמעות → מהיטמעות | מהפרדות → מהיפרדות | מהמלטות → מהימלטות | להטמעות → להיטמעות | הכרות → היכרות (כשהמשמעות acquaintance)
    ⚠ חריג: 'הכרה' (קוגניציה/הבנה) — לא לשנות.

14. התאמת פועל-נושא גם בסדר V-S (פועל לפני נושא) — לא רק בסדר S-V:
    כשפועל מופיע לפני נושאו, ההתאמה במין ובמספר עדיין חובה.
    דוגמאות:
      "מגיע זהות יהודית" → "מגיעה זהות יהודית" (זהות = נקבה)
      "נמצא תוצאות" → "נמצאות תוצאות" | "קם הילדים" → "קמו הילדים"
    הזהר במיוחד בפעלי קיום/הגעה: יש/אין/קם/בא/נמצא/הגיע/חסר/נראה.

15. חסר פועל עזר במבנה: [שם עצם + תואר + שם פועל]:
    כשרצף תיאורי מסתיים בשם פועל ללא פועל מקשר, סביר שחסר פועל עזר (יכול/עשוי/מסוגל/ראוי) או מילית ('שניתן', 'שאפשר').
    דוגמאות:
      "המדינות הכי מתקדמות להתקנא בה" → "המדינות הכי מתקדמות שניתן/שאפשר להתקנא בהן"
      "השיטה הטובה ביותר ליישם" → "השיטה הטובה ביותר שניתן ליישם"
    ⚠ חריג: כשהשם הפועל הוא חלק מסמיכות תיאורית תקינה ("רצון להתפתח", "כוונה ללמוד") — אל תיגע.

16. צירוף תיאורי קטוע (sentence fragment) ללא פועל מוטה:
    משפט המסתיים בנקודה ומורכב **רק** מצירוף תיאורי (תחילית 'ב-/ל-/מ-/בעקבות/בעת/בזמן/בהובלת/לאור') עם שמות עצם — ללא פועל מוטה — הוא קטוע ודורש תיקון.
    דוגמאות:
      "בהובלת עם ישראל קדימה." → צרף למשפט הסמוך עם מקף או פסיק: "בהובלת עם ישראל קדימה —" או "בהובלת עם ישראל קדימה,"
      "בעקבות ההחלטה הזאת." (אם עומד לבדו) → צרף למשפט סמוך
      "בהובלת עם ישראל והיהדות קדימה. זוהי החובה" → "בהובלת עם ישראל והיהדות קדימה — זוהי החובה" (החלף נקודה במקף)
    💡 כלל זיהוי: אם המשפט מתחיל ב-ב-/ל-/מ- ואין בו פועל בנטייה (רק שמות עצם ותוארים) — הוא קטוע.
    ⚠ אזהרת תיקון-יתר: כותרות עם נקודתיים, קריאות, סלוגנים, תגיות ('פסטיבל הקציר', 'יום העצמאות'), וכותרות ממוספרות שאחריהן יבוא תוכן — לא לשנות.
...
- **אסור לפצל משפט/פסקה לכמה שורות**: שורה שמכילה כמה משפטים (עם נקודות באמצע) חייבת להישאר שורה אחת בפלט. אסור להוסיף ירידת שורה אחרי כל נקודה.
- **אסור להחליף מילה במילה אחרת בעלת משמעות דומה**: רק תיקון שגיאות ממשיות. אם המילה תקינה — אל תיגע. דוגמאות אסורות: ייווכחו→ויוודאו, בחר→עסק, הינה→היא, מעיד→משקף.
- **אסור להפיל אותיות ממילה**: שמור כל אות. אסור: כבקשתך→בקשתך. אסור: בניגוד→ביוגוד. אם אתה לא בטוח שיש שגיאה — השאר בדיוק כמו במקור.
...
- "הינה/הינו" — מילים כשרות. אל תחליף ל"היא/הוא". שמור את הבחירה של הכותב.

══ עיקרון מנחה ══
אם משפט תקין דקדוקית — **אל תיגע בו**. תפקידך לתקן שגיאות, לא לשפר ניסוח. במקרה של ספק — תן עדיפות לשמירת המקור.
...
function normalizeStructuredSpacing(lines) {
  // סמן פסקה: שורה שמתחילה (אופציונלי עם */**) באות עברית / ספרה / אימוגי-ספרה
  const markerRe = /^(?:\*{1,2})?(?:[אבגדהוזחטיכלמנסעפצקרשת]|\d{1,2})(?:\*{1,2})?[.):\s]|^[0-9]️?⃣|^🔟/;
  const markerCount = lines.filter(l => markerRe.test(l.trim())).length;

  if (markerCount < 2) return lines; // מאמר לא-ממוספר — אל תיגע ברווחים

  // שלב א: קריסת שורות ריקות כפולות לאחת
  const collapsed = [];
  let prevBlank = false;
